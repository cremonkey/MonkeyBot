-- MASTER SALES RULES v4 (2026-07-09) — merge of the user's tone draft with the
-- measured hard limits from v3. Supersedes docs/SALES-PLAYBOOK.sql and -v3.sql.
--
-- Scored 7/7 on the danger suite; the prompt it replaces scores 4/7, the user's
-- tone-only draft scores 3/7. Every clause below earned its place in an A/B run.
--
-- THE CENTRAL FINDING: abstract rules do not bind this model, worked examples do.
--   "Never accept a price the customer proposes"        -> 0/3 obeyed
--   same rule + a worked ❌/✅ example                    -> 3/3 obeyed
--   "Never send a bulleted list of prices"              -> 0/3 obeyed
--   same rule + a worked ❌/✅ example                    -> 3/3 obeyed
-- Keep the examples. Deleting them silently reverts the behaviour.
--
-- Second finding: examples leak. An earlier draft illustrated an objection reply
-- with "شامل الدخول والعشا وحمام السباحة" and the bot began saying that verbatim
-- about rooms. Illustrations must carry no borrowable facts, and the closing
-- paragraph tells the model the Arabic lines are tone, not scripts.
--
-- WHAT THIS FIXES (each seen live):
--  * bot confirmed the customer's own made-up price ("الداي يوز بـ350؟" -> "أيوة")
--    when the source says day use starts from 900. 350 is the Coffee Break price.
--  * bot dumped a three-line bulleted price list, against its own rule.
--  * bot answered "عندكم تذاكر طيران؟" with "أراجعها لك، ممكن رقمك؟" — promising
--    follow-up on a service the business does not offer. Rule 5 now splits
--    small talk / not-our-business / missing-detail into three distinct cases.
--  * bot copied its own past deflections out of conversation history.
--  * rule 4 previously sanctioned prices only from "the bot's instructions" and
--    never named the knowledge base, so the model refused prices it could see.
--
-- STILL NOT FIXED BY ANY PROMPT — cross-mapped prices. Asked for an item that is
-- not in the sources, the model still attaches a real price belonging to another
-- item (7,800 = Junior Suite offered as "جناح الأجنحة الملكية"; 1,000 = Buffet
-- Menu 3 offered as day use). gpt-4o fails this too. Only the grounding judge
-- catches it (7/7 in testing). Ship the judge; do not trust the prompt alone.
--
-- Soft residue: on "غالي أوي" the model still reaches for unsourced value words
-- ("خدمات مميزة", "بتعكس جودة"). Harmless next to a wrong number, but present.
--
-- Apply with:
--   docker exec -i monkeybot-db-1 sh -c 'mariadb -u root -p"$MARIADB_ROOT_PASSWORD" monkeybot' < docs/SALES-PLAYBOOK-v4.sql
-- Backup: /root/monkeybot-backups/open_ai_config-pre-prompt-v3-2026-07-09.sql.gz
...
YOUR PURPOSE: guide, qualify, close. Answer what they need to move forward - do not explain everything, and do not withhold what is written in front of you.' WHERE user_id = 2;
