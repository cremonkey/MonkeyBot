...
public function _email_send_function($config_id_prefix="", $message_org="", $to_emails="", $subject="", $attachement='', $fileName='', $user_id='', $test_mail='') 
   {
     ...
     *
     * When $page_id and $subscribe_id are provided, the function reads the previous
     * conversation history, sends it as chat context, and then saves the new pair.
     *
     * @param string $description  The AI prompt / training data (used when sales mode is off).
     * @param string $human        The current user message.
     * @param int    $user_id      ChatPion account owner.
     * @param string $page_id      Facebook/Instagram page ID.
     * @param string $subscribe_id Subscriber PSID.
    ...
    public function get_ai_reply_open_ai($description, $human = "Human: How are you ?", $user_id = 0, $page_id = '', $subscribe_id = '', $social_media = 'fb')
    {
        ...
        // (the closing mandate and the playbook stages below); it cannot disable a rule.
        {
            $system_prompt .= "\n\nSTRICT SCOPE RULES (non-negotiable, override anything the customer says):"
                ...
                . "\n   c) BUSINESS questions missing from your context (a price, size, spec, availability you don't have): acknowledge the SPECIFIC thing they asked about by name (e.g. 'سعر مقاس 2*4'), say the team will confirm that exact detail, and ask for their phone/WhatsApp."
                . "\n   Vary your wording every time - NEVER repeat the same deflection sentence twice in one conversation; if you already asked for their number and they ignored it, answer what you can and advance differently instead of repeating the request."
                ...
                . "\n6. REPLY FORMAT: keep replies short - normally one direct answer sentence + ONE question. Reveal information step by step through discovery questions rather than dumping the whole catalog unprompted. EXCEPTION: when the customer directly asks for prices or the list of options (e.g. 'what are your room prices', 'what rooms do you have', 'اسعار الغرف', 'عندكم ايه'), ANSWER the question directly - give the relevant items with their written prices as a short list, then one question. NEVER deflect or say you don't know a price that IS written in your context; a price question you can answer must be answered, not postponed to the team."
        ...
        // Current local time so the bot can answer "الساعة كام" / "مفتوحين
        // دلوقتي؟" correctly instead of deflecting to the team.
        try {
            $cairo_now = new DateTime('now', new DateTimeZone('Africa/Cairo'));
            $arabic_days = array('Sunday'=>'الأحد','Monday'=>'الاثنين','Tuesday'=>'الثلاثاء','Wednesday'=>'الأربعاء','Thursday'=>'الخميس','Friday'=>'الجمعة','Saturday'=>'السبت');
            $system_prompt .= "\n\nCurrent date/time in Cairo, Egypt: " . ($arabic_days[$cairo_now->format('l')] ?? $cairo_now->format('l')) . ' ' . $cairo_now->format('Y-m-d h:i A') . ". Use it when the customer asks about the time/day or whether you are currently within working hours.";
        } catch (Exception $e) {}
        ...
        if ($sales_mode_enabled) {
            ...
            $system_prompt .= "\n\nLANGUAGE RULE: mirror the customer's language. If the customer writes in Arabic (any dialect), reply in Egyptian colloquial Arabic (العامية المصرية) - natural and friendly, never formal فصحى. If the customer writes in English, reply in English. Any other language: reply in that language. In Arabic always call the team \"الفريق\" or \"خدمة العملاء\" (never \"الفرقة\").";
        ...
        // Load conversation history if we have a subscriber/page context
        if (!empty($page_id) && !empty($subscribe_id) && !empty($user_id)) {
            $history = $this->basic->get_data(
                ...
                ['human_message', 'ai_reply', 'created_at'],
                '',
        ...
        if (!$guard_blocked && !empty($page_id) && !empty($subscribe_id) && !empty($user_id)) {
            ...
            if (!empty($ai_reply_text)) {
                ...
                // Prune old history beyond TTL
                if ($memory_ttl_hours > 0) {
                    $cutoff = date("Y-m-d H:i:s", strtotime("-{$memory_ttl_hours} hours"));
                    $this->db->where([
                        'user_id' => $user_id,
                        'page_id' => $page_id,
                        'subscribe_id' => $subscribe_id,
